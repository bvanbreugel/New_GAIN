...
def init_arg():
    ...
    parser.add_argumetn(
        '--mnist_visualise', default = False, type=bool, help='Saves some intermediate resulting imputations for MNIST')
...
if __name__ == '__main__':
    ...
    if use_default_GAIN_model:
        ...
        def generator(new_x,m):
            ...
            G_prob = tf.nn.sigmoid(tf.matmul(G_h3, G_W3) + G_b3) # [0,1] normalized Output
    ...
    #%% Loss
    D_loss1 = -tf.reduce_mean(tf.log(D_real + 1e-8) + tf.log(1. - D_fake + 1e-8)) 
    ...
    for it in pbar:    
        ...
        _, D_loss_curr = sess.run([D_solver, D_loss1], feed_dict = {M: M_mb, X:X_mb, New_X: New_X_mb, X_R: X_mbR, M_R: M_mbR})
        ...
        #Visualsie mnist results
        if dataset=='mnist' and mnist_visualise and (it%1000==0 or it in [0,20,50,100,200,400,1000,5000]):
            num_examples = 10 # num examples to display
            X_mb = trainX[:num_examples, :]
            Z_mb = sample_Z(num_examples, Dim)  # random noise between 0 and 0.01
            M_mb = trainM[:num_examples, :]  # mask mbsize
            New_X_mb = M_mb * X_mb + (1-M_mb) * Z_mb  # Missing Data Introduce
            imputed_data = sess.run([G_sample], feed_dict = {New_X:New_X_mb, M: M_mb})[0]
            imputed_data = M_mb * X_mb + (1-M_mb) * imputed_data
            loss = np.sqrt(np.sum((imputed_data-X_mb)**2)/np.sum(1-M_mb))
            print('current RMSE G loss',loss)
              # Renormalization
            if not os.path.exists('mnist_results'):
                os.mkdir(mnist_results)
            for i in range(num_examples):
                pixels = np.array(imputed_data[i])*255
                pixels = pixels.reshape((28, 28))
                plt.imshow(pixels, cmap='gray')
                folder_name ='mnist_results/intermediate_new_ex'+str(i)
                if not os.path.exists(folder_name):
                    os.mkdir(folder_name)
                plt.savefig(folder_name+'/imputed_iter'+str(it)+'.png')
                plt.close('all')
